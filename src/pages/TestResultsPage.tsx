...
import { 
  BarChart, PieChart, Clock, Award, Target, ArrowRight, 
  Download, Share2, Home, CheckCircle, X,
  AlertTriangle, TrendingUp, Compass
} from 'lucide-react';
...
const TestResultsPage = () => {
  ...
  useEffect(() => {
    ...
    if (location.state && typeof location.state === 'object' && 'testType' in location.state) {
      const state = location.state as TestResultsState;
      const { testType, score, totalQuestions, timeSpent, selectedAnswers } = state;
      
      // Calculate percentage correctly
      const calculatedPercentage = totalQuestions > 0 ? (score / totalQuestions) * 100 : 0;
  ...
  const getResultStatus = () => {
    if (testResults.percentage >= 80) return { label: 'Excellent', color: 'text-sage' };
    if (testResults.percentage >= 60) return { label: 'Good', color: 'text-terracotta' };
    if (testResults.percentage >= 40) return { label: 'Average', color: 'text-mutedGold' };
  ...
  const questionAnalysis = {
    ...
    incorrect: testResults.totalQuestions - testResults.score,
    unattempted: 0,
    timePerQuestion: formatTime(Math.floor(testResults.timeSpent / testResults.totalQuestions))
  };
      ...
      <div className="bg-deepNavy text-white relative overflow-hidden">
        ...
        <div className="container mx-auto px-4 py-16 relative z-10">
          ...
          >
            ...
            <h1 className="text-4xl md:text-5xl font-serif font-bold mb-4">Your Test Results</h1>
              ...
              <div className="flex flex-col md:flex-row items-center justify-between mb-8">
                <div className="mb-6 md:mb-0">
                  <h2 className="text-2xl font-serif font-bold text-deepNavy mb-2">
                    {testResults.testType === 'mcq' ? 'Multiple Choice Test Results' : 'Sketching Test Results'}
                  </h2>
                  <p className="text-charcoal/70">Completed on {new Date().toLocaleDateString()}</p>
                </div>
                    ...
                    <div className="relative w-48 h-48">
                      <svg className="w-full h-full" viewBox="0 0 100 100">
                        ...
                        <circle
                          ...
                          r="45"
                          fill="none"
                          stroke={testResults.percentage >= 80 ? '#8BA793' : 
                                  testResults.percentage >= 60 ? '#C66B3D' : 
                                  testResults.percentage >= 40 ? '#C4A484' : '#EF4444'}
                          strokeWidth="8"
                          strokeDasharray="283"
                          strokeDashoffset={283 - (283 * testResults.percentage) / 100}
                          strokeLinecap="round"
                      ...
                      <div className="absolute inset-0 flex flex-col items-center justify-center">
                        <span className="text-4xl font-bold text-deepNavy">{Math.round(testResults.percentage)}%</span>
                        <span className={`text-sm font-medium ${resultStatus.color}`}>{resultStatus.label}</span>
                      </div>
                    </div>
                  ...
                  <div className="col-span-2">
                        ...
                        <ul className="space-y-2">
                          {improvements.map((improvement, index) => (
                            <li key={index} className="flex items-start">
                              ...
                              <span className="text-charcoal/80">{improvement}</span>
                            </li>
                          ))}
                        </ul>
                      </div>
                    ...
                    <div className="mt-6 bg-cream rounded-lg p-4">
                      ...
                      <div className="grid grid-cols-2 md:grid-cols-4 gap-4 text-center">
                        <div>
                          <div className="text-xl font-bold text-sage">{questionAnalysis.correct}</div>
                          <div className="text-sm text-charcoal/70">Correct</div>
                        </div>
                        <div>
                          <div className="text-xl font-bold text-terracotta">{questionAnalysis.incorrect}</div>
                          <div className="text-sm text-charcoal/70">Incorrect</div>
                        </div>
                        <div>
                          <div className="text-xl font-bold text-mutedGold">{questionAnalysis.unattempted}</div>
                          <div className="text-sm text-charcoal/70">Unattempted</div>
                        </div>
                        <div>
                          <div className="text-xl font-bold text-deepNavy">{questionAnalysis.timePerQuestion}</div>
                          <div className="text-sm text-charcoal/70">Avg. Time/Q</div>
                        </div>
                      </div>
                    </div>
                  </div>
              ...
              {testResults.testType === 'sketching' && (
                <div className="text-center">
                  <div className="mb-6">
                    ...
                    <h3 className="text-xl font-medium text-deepNavy">Submission Successful</h3>
                    <p className="text-charcoal/70 mt-2">
                      Your sketching test has been submitted successfully. Our team will review your drawings and provide feedback within 48 hours.
                    </p>
                  </div>
                  
                  <div className="bg-cream rounded-lg p-6 max-w-md mx-auto">
                    <h4 className="font-medium text-deepNavy mb-3">Submission Details</h4>
                    <div className="space-y-2 text-left">
                      <div className="flex justify-between">
                        <span className="text-charcoal/70">Test Type:</span>
                        <span className="font-medium text-deepNavy">Sketching Test</span>
                      </div>
                      <div className="flex justify-between">
                        <span className="text-charcoal/70">Submission Date:</span>
                        <span className="font-medium text-deepNavy">{new Date().toLocaleDateString()}</span>
                      </div>
                      <div className="flex justify-between">
                        <span className="text-charcoal/70">Time Spent:</span>
                        <span className="font-medium text-deepNavy">{formatTime(testResults.timeSpent)}</span>
                      </div>
                      <div className="flex justify-between">
                        <span className="text-charcoal/70">Tasks Completed:</span>
                        <span className="font-medium text-deepNavy">{testResults.totalQuestions}</span>
                      </div>
                    </div>
                  </div>
                </div>
